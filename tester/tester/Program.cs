...
class HanoiTower
{
    ...
    public bool IsTowerSorted(int towerIndex)
    {
        ...
        var values = towers[towerIndex].ToArray();
    ...
    private bool IsValidMove(int source, int target)
    {
        ...
        if (towers[target].Count == 0)
            return true;
        return towers[source].Peek() < towers[target].Peek();
    }
    ...
    private void DisplayState()
    {
        ...
        int[,] display = new int[n, 3];

        for (int i = 0; i < 3; i++)
        {
            int[] towerArray = towers[i].ToArray();
